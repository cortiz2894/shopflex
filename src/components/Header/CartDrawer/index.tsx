...
export default function CartDrawer({ isCartOpen, toggleCart }: CartDrawerProps) {
  ...
  return (
    <div
      className={classNames('h-[100vh] w-full md:w-[31.5vw] fixed top-0 z-[999999]', [styles.drawer], {
        [styles.active]: isCartOpen,
      })}
    >
      <Curve active={isCartOpen} />
      <div
        className={classNames('bg-white z-10 top-0 flex flex-col', [styles.content], { [styles.active]: isCartOpen })}
      >
        ...
        <div className={classNames('p-6', [styles.header])}>
          <Title text="Your bag" />
          <span className="text-standar-darker px-2">
            <b>{totals.quantity}</b>
        ...
        >
          {products.length === 0 && (
            <p className="text-standar-darker flex justify-center h-full items-center">Your Cart is Empty</p>
          )}
          {products.map((prod, index) => (
            ...
            >
              <div className={classNames([styles.imageContainer], 'mt-4 mb-4')}>
                <Image src={getImage(prod.image)} width={500} height={500} objectFit="none" alt="clothes" />
              </div>
              <div className="w-3/4 flex flex-col justify-around relative mt-4 mb-4">
                ...
                <button
                  className="text-standar-darker absolute top-0 right-0"
                  onClick={() => handleRemoveProduct(prod.id, index)}
                >
        ...
        {products.length > 0 && (
          <div className="bg-black p-6 absolute bottom-0 right-0 w-full">
            <div className="flex justify-between w-full mb-2">
              <h3 className="text-white text-3xl">Total</h3>
              ...
              <ButtonPrimary
                ...
                text={
                  <span className={'flex relative'}>
                    Checkout
                    <AiOutlineLock className="text-[20px] ml-2" />
                  </span>
                }
              />
            </div>
