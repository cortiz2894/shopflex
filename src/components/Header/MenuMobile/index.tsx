  ...
  return (
    <>
      ...
      >
        ...
        >
          {navlinks.map((link, index) => (
            <li
              ...
              onClick={() => link.dropdown && selectNavlink(link.title)}
            >
        ...
        <div
          ref={openItemSlideRef}
          className={classNames(styles.openItemSlide, { [styles.active]: openSlide }, 'p-3 pt-8')}
        >
          ...
          <ul className="flex flex-col gap-4 translate-y-6 opacity-0" ref={openItemContentRef}>
            {selectedNavlink?.dropdown?.map((dropdownItem, index) => (
              <li key={index} className="text-standar-darker text-lg">
                <p>{dropdownItem.title}</p>
              </li>
            ))}
          </ul>
          <div
            className={classNames('absolute left-0 bottom-0 p-3 w-full h-[35vh]', [styles.imageDropdown], {
              [styles.active]: openSlide,
            })}
          >
            <Image src={`/images/${selectedNavlink?.img}`} layout="fill" objectFit="cover" alt="clothes" />
            <span className="text-white text-lg absolute z-10 top-3 left-3">{selectedNavlink?.title}</span>
            <button className="bg-white text-standar-darker px-3 py-2 text-sm absolute z-10 bottom-3 left-3">
              <LinkTransition href={'/products'}>See all</LinkTransition>
            </button>
          </div>
        </div>
      </div>
