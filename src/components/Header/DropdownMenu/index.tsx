    ...
    <>
      <div
        className={classNames(
          'w-11/12 rounded bg-transparent menu-dropdown z-10 flex justify-center overflow-hidden relative',
          { active: showDropdown },
        )}
        onMouseLeave={() => {
        ...
        <div className="w-11/12 flex justify-between">
          <div className="w-1/2 py-5">
            <p className="text-2xl text-black mb-3">
              {hoveredIndex !== null && navlinks[hoveredIndex as number].title}
            </p>
            <ul className="pl-2 w-auto">
              {hoveredIndex !== null &&
                navlinks[hoveredIndex].dropdown &&
                navlinks[hoveredIndex].dropdown!.map((item: NavlinkDropdown, index: number) => (
                  <li key={index} className="dropdown-item mb-2 relative">
                    <button
                      className={classNames(
                        'font-inter text-standar-lighter hover:text-standar-lighter text-xl bg-none border-none',
                        {
                          'blur-text':
                            (hoveredButtonIndex !== index && hoveredButtonIndex !== null) ||
                            (activeSubmenu !== index && activeSubmenu !== null),
                        },
                    ...
                    <div
                      className={`submenu submenu-${index} overflow-hidden lg:w-auto lg:absolute top-0 lg:left-[25%] z-50 flex flex-col items-start ml-10 lg:r-ml-20 gap-2 ${activeSubmenu === index ? 'active' : ''}`}
                    >
                      {item.types &&
                        item.types.map((type, typeIndex) => (
                          <span key={typeIndex} className="text-standar-darker relative">
                            {type.title}
                          </span>
                        ))}
                    </div>
