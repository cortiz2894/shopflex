...
export default function CarouselImages({ images, isLite = false }: Props) {
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    ...
    gsap.to(magnifyGlassElem, {
      backgroundPosition: `calc(${positionRatio[0] * 100}% - ${offset[0]}px) calc(${positionRatio[1] * 100}% - ${
        offset[1]
      }px)`,
      top: `${offsetY}px`,
      left: `${offsetX}px`,
  ...
  return (
    <>
      <div className={classNames(styles.image, { '!h-full': isLite })}>
        ...
        <div className="absolute top-2 right-[calc(15%+0.75rem)] z-[99999] hidden md:block">
          <ButtonPrimary
            theme="light"
            size="small"
            variant="lessRounded"
        ...
        <div
          className={classNames('relative flex gap-0 h-full md:gap-3', [styles.selectedImageContainer])}
          ...
          <>
            {images.map((image, i) => (
              ...
              >
                ...
                ></div>
                <Image src={getImage(image)} layout="fill" objectFit="cover" alt={'Reign-Of-Blood'} key={i} />
              </div>
            ))}
          </>
        </div>
        {selected < images.length - 1 && (
          <button
            ...
            className={classNames('absolute right-0 h-full top-0 z-10', [styles.arrow], [styles.right])}
          >
            <BsChevronCompactRight className={classNames('text-[3em] text-white', [styles.svgPrime])} />
            <span className={styles.animatedButton}>
              <BsChevronCompactRight className={'text-[3em] text-white'} />
            </span>
          </button>
        )}
        {selected === images.length - 1 && (
          <button
            ...
            className={classNames('absolute left-0 h-full top-0 z-10', [styles.arrow], [styles.left])}
          >
            <BsChevronCompactLeft className={classNames('text-[3em] text-white', [styles.svgPrime])} />
            <span className={styles.animatedButton}>
              <BsChevronCompactLeft className={'text-[3em] text-white'} />
            </span>
          </button>
        )}
      </div>
